...
var BattleTank = function() {
	...
	var Level = function(levelDetails) {
		...
		this.describeTask = function(task) {
			...
			switch(action) {
				case bt.ACTION_COLLECT:
					html += (object == bt.ITEM_TYPE_CHECKPOINT?'行く→':'集める→');
					break;
		...
		this.updateDetails = function() {
			...
			if(!l)
				...
				html = '<a id="restartLevel" data-icon="back" data-role="button">レベルを再実行する</a>';
				if(l.remainingTasks.length == 0) {
					html += '<a id="nextLevel" data-icon="forward" data-role="button">次のレベル</a>';
				}
				html += '<h1>' + l.title + '</h1>' + l.description;
			
				
				html += '<h2>' + l.completedTasks.length + ' タスク完了</h2><ul class="list_tick">';
				for(var i = 0; i < l.completedTasks.length; i++) {
					html += '<li>' + this.describeTask(l.completedTasks[i]) + '</li>';
				}
				html += '</ul>';
				
				html += '<h2>' + l.remainingTasks.length + ' タスク残</h2><ul class="list_cross">';
				for(var i = 0; i < l.remainingTasks.length; i++) {
					html += '<li>' + this.describeTask(l.remainingTasks[i]) + '</li>';
				}
				html += '</ul>';
			}
			html += '<a id="btn_choose_level" data-icon="grid">レベルを選択</a>';
			$('#objectives').html(html);
			
			// choose level
			$('#btn_choose_level').button().parent().click(function() {
				var html = '<h2>レベルを選択</h2><div class="white">Levels:';
			...
			$('#restartLevel').button().parent().click(function(e, ui) {
				var html = '<div><h2>CPUコードとレベルをリセットしますか？</h2><div class="white"><p>「はい」をクリックすると、それぞれの戦車のCPUに書き込まれたコードをこのレベルのデフォルトにリセットします。</p><p>「いいえ」をクリックすると、現在の戦車のCPUのコードを維持します。</p><p>「キャンセル」をクリックすると、レベルの再実行をキャンセルします。</p></div><p><a id="btn_reset_yes" data-icon="check" data-inline="true">はい</a><a id="btn_reset_no" data-icon="delete" data-inline="true">いいえ</a><a id="btn_reset_cancel" data-icon="back" data-inline="true">キャンセル</a></p></div>';
		...
		this.showNextTask = function() {
			...
			if(this.remainingTasks.length > 0) {
				...
				var msg = percent + "% 完了。次のタスク: " + this.describeTask(nextTask);
				...
				bt.grid.updateStatus("おめでとう: レベルが完了しました。");
	...
	var Tank = function(tankId, x, y, color, grid) {
		...
		this.getInput = function(inputDevice, accumulator) {
			...
			switch(inputDevice) {
			case 0:	// User input
				val = parseInt(window.prompt("入力→ " + tank.tankId + ": "), 10);
				if(isNaN(val))
					val = 0;
				break;
		...
		this.showCPU = function() {
			...
			var html = '<div id="ram"><h2>Little Man Computer CPU<a data-icon="delete" data-inline="true" data-mini="true" id="btn_editor_cancel">キャンセル</a></h2>';
			
			
			html += '<div><div data-collapsed="false" id="ram_editor_holder"><h2>コードエディタ</h2><a data-icon="gear" data-inline="true" data-mini="true" id="btn_editor_assemble">アセンブル</a><a data-icon="info" data-inline="true" data-mini="true" id="btn_editor_help" href="#help">ヘルプ</a><div id="ram_editor">' + code + '</div><span id="code_status">Little Man Computer</span></div>'
			html +='<div id="ram_contents_holder" data-collapsed="false"><h2>メモリ</h2><div id="ram_contents">';
			...
			html +='</div></div>';
			html += '<div id="control_holder"><div id="input" data-role="collapsible" data-collapsed="true"><h2>入力</h2><table class="device_table"><tr><th>入力デバイス:</th><th>アキュムレータ</th></tr>';
			html +='<tr><td><select data-mini="true" name="select_input_device" id="select_input_device"><option value="0">0: ユーザ入力</option><option value="1">1: ナビゲーションシステム</option><option value="2">2: 衝突レーダー</option><option value="3">3: 備蓄</option></select></td><td><select data-mini="true" name="select_input_accumulator" id="select_input_accumulator"><option>任意</option></select></td></tr></table><div id="input_explanation">ユーザから値を読み込む</div>'
			html +='<a id="btn_current_input_value" data-iconpos="right" data-inline="true" data-mini="true" data-icon="refresh">現在の値: 0</a>例:<textarea id="input_code_sample" class="code_sample">INP 0 // asks the user to enter a number</textarea></div>'
			html += '<div id="process" data-collapsed="false" data-role="collapsible"><h2>プロセス</h2>'
			html += '<h3>レジスタ</h3><div id="registers">'
			html += '<span id="pc_register" class="tank_pc register">プログラムカウンタ:<input type="text" name="pc_register_value" maxlength="3" size="3" class="register_value" id="pc_register_value" value="0"></span>';
			html += '<span id="ins_register" class="tank_code register">インストラクション:<input type="text" name="ins_register_value" maxlength="3" size="3" class="register_value" id="ins_register_value" value="0"></span>';
			html += '<span id="ac_register" class="tank_ac register">アキュムレータ:<input type="text" name="ac_register_value" maxlength="3" size="3" class="register_value" id="ac_register_value" value="0"></span>';
			html += '<h3>フェッチ・実行サイクル</h3>';
			html += '<a id="btn_run_restart" data-role="button" data-mini="true" data-inline="true" data-icon="back">再実行</a>';
			html += '<a id="btn_run_step" data-role="button" data-mini="true" data-inline="true" data-icon="forward">ステップ</a>';
			html += '<img id="fetch_execute_cycle" src="media/fetch_decode_execute.png"/><div id="control_status"></div>';
			html += '</div></div>';
			html += '<div id="output" data-collapsed="true" data-role="collapsible"><h2>出力</h2><table class="device_table"><tr><th>出力デバイス:</th><th>アキュムレータ</th></tr><tr><td><select data-mini="true" name="select_output_device" id="select_output_device"><option value="0">0: ユーザ出力</option><option value="1">1: 移動</option><option value="2">2: 方向転換</option><option value="3">3: 大砲の向き</option><option value="4">4: 大砲</option></select></td><td><select data-mini="true" name="select_output_accumulator" id="select_output_accumulator"><option>任意</option></select></td></tr></table><div id="output_explanation">ユーザに値を送る</div><a id="btn_current_output_value" data-iconpos="right" data-inline="true" data-mini="true" data-icon="refresh">テスト</a>例:<textarea id="output_code_sample" class="code_sample">OUT 0 // display a number</textarea></div>';
			...
			var updateCurrentInputValue = function() {
				...
				$('#btn_current_input_value').html("現在の値: " + val);
			};
				...
				case "1": // Tank direction
					setInputAccumulatorOptions([
						{ 
							value: '0', 
							text: 'GetX', 
							description: '現在のX座標をアキュムレータレジスタに保存する', 
							example:'LDA GetX\nINP 1 // Navigation\nHLT // rest of program goes here\nGetX  DAT 0'
						}, 
						{ 
							value: '1', 
							text: 'GetY', 
							description: '現在のY座標をアキュムレータレジスタに保存する', 
							example:'GetY\nINP 1 // Navigation\nHLT // rest of program goes here\nGetY  DAT 1'
						},
						{ 
							value: '2', 
							text: 'GetTankDir', 
							description: '現在の戦車の向きをアキュムレータレジスタに保存する<table class="device_table"><tr><th>アキュムレータ</th><th>意味</th></tr><tr><td>0</td><td>北</td></tr><tr><td>2</td><td>東</td></tr><tr><td>4</td><td>南</td></tr><tr><td>6</td><td>西</td></tr></table>',
							example: 'LDA TankDir\nINP 1 // Navigation\nHLT // rest of program goes here\nTankDir DAT 2'
						},
						{ 
							value: '3', 
							text: 'GetGunDir', 
							description: '大砲の向きを取得する<table class="device_table"><tr><th>アキュムレータ</th><th>意味</th></tr><tr><td>0</td><td>北</td></tr><tr><tr><td>1</td><td>北東</td></tr><tr><td>2</td><td>東</td></tr><tr><td>3</td><td>南東</td></tr><tr><tr><td>4</td><td>南</td></tr><tr><td>5</td><td>南西</td></tr><tr><tr><td>6</td><td>西</td></tr><tr><td>7</td><td>北西</td></tr><tr></table>',
							example: 'LDA GunDir\nINP 1 // Navigation\nHLT // rest of program goes here\nGunDir DAT 3'
						}]);
					break;
				...
				case "2": // Collision radar
					setInputAccumulatorOptions([
						{ 
							value: '0', 
							text: 'MoveForwards?', 
							description: '戦車が前進可能かチェックする<table class="device_table"><tr><th>アキュムレータ</th><th>意味</th></tr><tr><td>0</td><td>障害物あり - 前進不能</td></tr><tr><tr><td>1</td><td>前進可能</td></tr></table>', 
							example:'LDA MoveForwards\nINP 2 // Collision radar\nHLT // rest of program goes here\nMoveForwards DAT 0'
						},
						{ 
							value: '1', 
							text: 'MoveBackwards?', 
							description: '戦車が後進可能かチェックする<table class="device_table"><tr><th>アキュムレータ</th><th>意味</th></tr><tr><td>0</td><td>障害物あり - 後進不能</td></tr><tr><tr><td>1</td><td>後進可能</td></tr></table>', 
				...
				case "3": // Supplies
					setInputAccumulatorOptions([
						{ 
							value: '0', 
							text: 'GetAmmo', 
							description: '弾薬の量をアキュムレータレジスタに保存する', 
							example:'LDA Ammo\nINP 3 // Supplies\nHLT // rest of program goes here\nAmmo DAT 0'
						},
						{ 
							value: '1', 
							text: 'GetFuel', 
							description: '燃料の量をアキュムレータレジスタに保存する', 
							example:'LDA Fuel\nINP 3 // Supplies\nHLT // rest of program goes here\nFuel DAT 0'
						}]);
					break;
				}				
			...
			$('#select_output_device').selectmenu().on("change", function() {
				switch(this.value) {
				case "0":	// User output
					setOutputAccumulatorOptions([{ value: '0', text: '任意', description: '数字を表示する', example:'INP // ask for a number\nOUT // display the number' }]);
					break;
				...
				case "1": // Movement
					setOutputAccumulatorOptions([
						{ 
							value: '0', 
							text: '前進', 
							description: '戦車を1スペース前進させる', 
							example:'LDA Forwards\nOUT 1 // Move\nHLT // rest of program goes here\nForwards DAT 0'
						}, 
						{ 
							value: '1', 
							text: '後進', 
							description: '戦車を1スペース後進させる', 
				...
				case "2": // Steer tank
					setOutputAccumulatorOptions([
						{ 
							value: '0', 
							text: '北', 
							description: '戦車を北に向かせる', 
							example:'LDA North\nOUT 2 // Steer\nHLT // rest of program goes here\nNorth DAT 0'
						},
						{ 
							value: '2', 
							text: '東', 
							description: '戦車を東に向かせる', 
							example:'LDA East\nOUT 2 // Steer\nHLT // rest of program goes here\nEast DAT 2\n'
						},
						{ 
							value: '4', 
							text: '南', 
							description: '戦車を南に向かせる', 
							example:'LDA South\nOUT 2 // Steer\nHLT // rest of program goes here\nSouth DAT 4'
						},
						{ 
							value: '6', 
							text: '西', 
							description: '戦車を西に向かせる', 
							example:'LDA West\nOUT 2 // Steer\nHLT // rest of program goes here\nWest DAT 6'
						}
						]);
					break;
				...
				case "3": // Aim gun
					setOutputAccumulatorOptions([
						{ 
							value: '0', 
							text: '北', 
							description: '大砲を北に向ける', 
							example:'LDA North\nOUT 3 // Aim\nHLT // rest of program goes here\nNorth DAT 0'
						},
						{ 
							value: '1', 
							text: '北東', 
							description: '大砲を北東に向ける', 
							example:'LDA NorthEast\nOUT 3 // Aim\nHLT // rest of program goes here\nNorthEast DAT 1'
						},
						{ 
							value: '2', 
							text: '東', 
							description: '大砲を東に向ける', 
							example:'LDA East\nOUT 3 // Aim\nHLT // rest of program goes here\nEast DAT 2\n'
						},
						{ 
							value: '3', 
							text: '南東', 
							description: '大砲を南東に向ける', 
							example:'LDA SouthEast\nOUT 3 // Aim\nHLT // rest of program goes here\nSouthEast DAT 3'
						},
						{ 
							value: '4', 
							text: '南', 
							description: '大砲を南に向ける', 
							example:'LDA South\nOUT 3 // Aim\nHLT // rest of program goes here\nSouth DAT 4'
						},
						{ 
							value: '5', 
							text: '南西', 
							description: '大砲を南西に向ける', 
							example:'LDA SouthWest\nOUT 3 // Aim\nHLT // rest of program goes here\nSouthWest DAT 5'
						},
						{ 
							value: '6', 
							text: '西', 
							description: '大砲を西に向ける', 
							example:'LDA West\nOUT 3 // Aim\nHLT // rest of program goes here\nWest DAT 6'
						},
						{ 
							value: '7', 
							text: '北西', 
							description: '大砲を北西に向ける', 
			...
			$('#btn_run_restart').button().parent().click(function() {
				...
				updateRegisters();
				gotoPCLine();
				$('#control_status').html('プログラムカウンタレジスタは0にセット');
			});
			...
			var checkCode = function() {
				$("#code_status").html('<span class="spinner"> </span>コードをチェック中...').css({'background-color':'rgba(255,255,200,0.7)', 'box-shadow':'0px 0px 10px rgba(255,255,200,0.7)'});
				if(checkTimeout) {
				...
				checkTimeout = setTimeout(function(e) {
					...
					if(result.success) {
						success = true;
						$("#code_status").html('<img src="media/tick.png" />エラーなし: ' + result.instructions.length + " % メモリ使用").css({'background-color':'rgba(200,255,200,0.7)', 'box-shadow':'0px 0px 10px rgba(200,255,200,0.7)'});
						...
						$("#code_status").html('<img src="media/cross.png" />' + result.errors.length + " 個のエラー: 次の行をチェック" + result.errors[0].line).css({'background-color':'rgba(255,200,200,0.7)', 'box-shadow':'0px 0px 10px rgba(255,200,200,0.7)'});
					}
				}, 500);
			}
		...
		this.updateStats = function() {
			if(bt.settings.showFuelDetails)
				$('#' + this.tankId + " .tank_fuel").html('燃料: ' + this.fuel + "&frasl;" + this.maxFuel);
			if(bt.settings.showAmmoDetails)
				$('#' + this.tankId + " .tank_ammo").html('弾薬: ' + this.ammo + "&frasl;" + this.maxAmmo);
	...
	var Grid = function(size){
		...
		// setup game
		this.init = function() {
			this.updateStatus("BattleTanksへようこそ");
			this.drawGrid();
			
			// setup tank movement callback timer
			setInterval(this.moveTanks, 1000);
			html = '<div id="welcome"><h3>CPU BattleTanksへようこそ</h3></h3><p>このパズルは、中央処理装置においてフェッチ / 実行サイクルがどのように動作しているのかを理解する手助けになるよう設計されています。</p><p>練習レベルでは、手動コントローラを使って戦車を動かすことができます。しかし、本当に面白くなるのは、低レベルなコードを書いて戦車をコントロールするようになってからです。</p><p>グリッドをクリックして開始しましょう。</p></div>';			
			$('#dlg').html(html);
		...
		this.drawGrid =  function() {
			...
			$('.grid_cell').click(function(e) {
				...
				for(var i = 0; i < grid.mapData[y][x].items.length; i++) {
					...
					case bt.ITEM_TYPE_ROCK:
							html += '<img class="item_description" src="media/rock.png" />岩: 障害物ですが、破壊したり通過することはできます';
						break;
					case bt.ITEM_TYPE_TANK:
							html += '<img class="item_description" src="media/tank-turret.png" />戦車: あなたが破壊できる戦車です。';
						break;
					case bt.ITEM_TYPE_FUEL:
						html += '<img class="item_description" src="media/fuel.gif" />燃料庫: ここへ戦車を動かすと燃料が補給されます';
						break;
					case bt.ITEM_TYPE_CRATER:
						html += '<img class="item_description" src="media/crater.png" />クレータ: 以前にここで何かが爆発したことを示します。';
						break;
					case bt.ITEM_TYPE_STRAW:
					...
					case bt.ITEM_TYPE_EXPLOSION:
						...
						break;
					case bt.ITEM_TYPE_CHECKPOINT:
						html += '<img class="item_description" src="media/checkpoint.gif" />チェックポイント: このマークがあるグリッドへ戦車を動かすことが目標です';
						break;
					case bt.ITEM_TYPE_AMMO:
						html += '<img class="item_description" src="media/ammo.gif" />Ammo dump: Drive your tank here to top up on ammunition';
						break;
					case bt.ITEM_TYPE_WATER:
						html += '<img class="item_description" src="media/water.png" />川: 川を挟んだ向こう側に大砲を射つことはYできますが、渡ることはできません';
						break;
					case bt.ITEM_TYPE_MINE:
						html += '<img class="item_description" src="media/mine.gif"/>地雷: 戦車がこの上に乗ると爆発します';
						break;
					}
				}
	...
	this.addTank = function(id, x,y, color, code) {
		var html = '<div id="tank_' + id + '" class="tank">\
			...
			<div class="tank_quote" id="tank_quote_tank_' + id + '">クリックしてコードを編集</div>\
	...
	this.setupController = function() {
		...
		if(this.settings.controller == this.CONTROLLER_LITTLE_MAN_COMPUTER) {
			if($('#cpu_controller').length == 0) {
				...
				$('#controller').html('<div id="cpu_controller"><a data-inline="true" data-icon="check" id="btn_run">実行</a><a data-inline="true" data-icon="forward" id="btn_step">1ステップ</a><a data-inline="true" data-icon="delete" id="btn_stop">停止</a><a data-inline="true" data-icon="back" id="btn_reset">リセット</a></div>').unbind();
